...
namespace MangaChecker.Utility {
    internal static class ImageLinkCollecter {
        public static List<string> YomangaCollectLinks(string url) {
            url = url + "page/1";
            var html = Tools.DownloadData(url) ?? CloudflareGetString.Get(url);
            ...
            for (var i = 1; i <= lastChapterNumber; i++) {
                ...
                var htmlimg = Tools.DownloadData(newlink.Replace("http:/yo", "http://yo")) ??
        ...
        public static List<string> MangastreamCollectLinks(string url) {
            ...
            for (var i = 1; i <= lastChapterNumber; i++) {
                ...
                var htmlimg = Tools.DownloadData(newlink.Replace("http:/man", "http://man")) ??
